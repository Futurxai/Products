...
- `offline-banner/` — `OfflineBannerComponent` (`app-offline-banner`, M5 Phase 5), mounted once in `AppComponent` next to `app-toast-host` — reads `core/network/network-status.service.ts`'s signal and shows an advisory (never blocking) banner while the browser reports itself offline.
