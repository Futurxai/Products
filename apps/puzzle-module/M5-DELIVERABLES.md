...
See `README.md` → End-to-End UAT for the full write-up. `e2e/creator-to-recipient.spec.ts` (Playwright) drives the real browser UI against real Firebase emulators through the complete Creator-to-Recipient loop — sign-up, wizard authoring (image upload/crop, 9 questions with clues), publish, and Recipient gameplay covering a direct-answer path, a wrong-answer-then-clue path, and a wrong-answer-then-partner-help path — reaching the final reveal screen with the exact expected score (785/900, 2 stars). It passes.

**This is the single most valuable thing M5 produced.** It found two real, production-blocking bugs that every other test layer in this project — 550 Karma specs, 171 functions unit specs, 46 real-emulator Firestore/Auth/Storage tests — had missed, because none of them exercised a real `<form>` element or real Firestore Rules together in one place:

1. **The Recipient could never actually submit an answer.** `question-modal.component.ts` (and its structural twin, the Creator's own Preview modal) never imported `FormsModule`, so `<form (ngSubmit)="onSubmit()">` — a bare form with no `[formGroup]`, since the answer field is a standalone `[formControl]` — had no directive listening for the native `submit` event. Clicking "Submit answer" (or pressing Enter) did precisely nothing, for any real user, since this component existed. The Karma spec for this component was green throughout M3–M5 because it called `onSubmit()` directly instead of dispatching a DOM event.
2. **A real Creator could never start a puzzle.** `FirestoreExperienceRepository.create()` wrote the new draft's public and private documents in one atomic `writeBatch`; the private doc's Firestore Rules `create` check reads the public doc via `get()` to establish ownership, but a batch evaluates all its writes against the pre-batch database state — so the public doc doesn't exist yet when the private doc's rule runs, and the whole batch is denied by Rules every time. This was invisible to the Cloud Functions emulator tests (Admin SDK, bypasses Rules) and to Karma (mocks the repository).

Both are fixed (see `README.md`'s End-to-End UAT section for the exact diffs), and both Karma specs gained a regression-guard test that dispatches a real DOM `submit` event rather than calling the handler directly, so this specific failure mode can't silently reappear.
...
- [x] End-to-end UAT (`npm run test:e2e`) covering the full Creator-to-Recipient journey against real emulators — found and fixed two production-blocking bugs (see End-to-End UAT above). Not wired into CI (needs a real Chromium binary and ~45s of browser automation); run manually before a release per `DEPLOYMENT.md`'s Release Checklist.
