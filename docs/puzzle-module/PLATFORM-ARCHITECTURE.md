...
> **⚠️ Out of date since [ADR-0011](adr/0011-dedicated-firebase-project-for-puzzle-module.md).** This document's foundational premise — "one shared Firebase project (`lovedigitally-app`), not a project per app," cited throughout §1, §3, §8, §11, §12 as the existing pattern to generalize — no longer holds. The Puzzle Module was migrated to its own dedicated project, `lovedigitally-puzzle`. This has real consequences for the platform proposal below that have **not** been re-worked here: a shared Auth user pool (§3) and a shared Storage bucket (§8) both assumed one project; a future module built directly against this document's "already works this way" claims would build against a premise that's no longer true for the Puzzle Module specifically. Treat every reference to `lovedigitally-app` below as historical context for how the proposal was originally reasoned about, not as current fact — this document needs a real revision pass before it's used to plan a second module, not just this one callout.
